...
class BaseTextCategorizationDataset:
    ...
    def get_index_to_label_map(self):
        ...
        """
        return {i : self._get_label_list()[i] for i in range(0, len(self._get_label_list()))}
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        def filter_tag_position(position):
            def filter_function(df):
                ...
                """
                position0=df['tag_position']==position
                filter_function_ = df[position0]
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                filter_function= df[df.tag_name.map(df['tag_name'].value_counts()) >= x]
                return filter_function
                # TODO: CODE HERE
        ...
        preprocessed_df=df.pipe(filter_tag_position(0)).pipe(filter_tags_with_less_than_x_samples(min_samples_per_label))
